...
public class OnlineFaceDemo extends Activity implements View.OnClickListener {
    ...
    public void onClick(View view) {
        ...
        if (viewId == R.id.online_pick){
            Intent intent = new Intent();
            intent.setType("image/*");
            intent.setAction(Intent.ACTION_PICK);
            startActivityForResult(intent, REQUEST_PICTURE_CHOOSE);
        }else if (viewId == R.id.online_reg){
            if (TextUtils.isEmpty(mAuthid)) {
                showTip("authid不能为空");
                return;
            }
        ...
        }else if (viewId == R.id.online_verify){
        ...
        }else if (viewId == R.id.online_camera){
            // 设置相机拍照后照片保存路径
        ...
        }else if (viewId == R.id.btn_modle_delete){
            // 人脸模型删除
            mModelCmd = MODEL_DEL;
            executeModelCommand("delete");
        }else if (viewId == R.id.btn_identity){
